...
class GaussianDiffusion_AR(Module):
    ...
    def model_predictions(self, x, t, x_self_cond = None, clip_x_start = True, rederive_pred_noise = False):
...
class Trainer_ARs:
    def __init__(
        ...
        *,
        ...
        train_datasets = [(f'/{folder}/lead_{i}/train.nc') for i in lead_nums]
        train_datasets = train_datasets[:2]
        config = get_config()
        self.ds = DataProcessed(file_paths=train_datasets, config=config, mean= 152.16729736328125, std= 147.735107421875, maxivt = 15.220598, minivt = -1.0300009)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    dataX = next(self.dl)[0].to(device).unsqueeze(1)
                    datay = next(self.dl)[1].to(device).unsqueeze(1)
                    with self.accelerator.autocast():
                        loss = self.model(dataX)
